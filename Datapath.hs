...
datapath ctlsigs memdat = (ma,md,cond,a,b,ir,pc,ad,ovfl,r,x,y,p,mul_ready) -- new input mul_ready for task 2 is added to the datapath
  where
...
-- Registers
      (a,b) = regfile n k (ctl_rf_ld ctlsigs) (mux1w (ctl_rf_increment ctlsigs) ir_d ir_sa) rf_sa rf_sb p -- for task 1, if the rf increment signal is 0 then use ir_d as input, else if 1 use ir_sa
...
-- Multiplier for task 2
      (mul_ready,prod,rx,ry,s) = multiply n (ctl_mul_start ctlsigs) x y
...
-- Internal processor signals
      ...
      p  = mux1w (ctl_rf_pc ctlsigs)                -- regfile data input
            (mux1w (ctl_rf_multi ctlsigs)             -- for task 2
              (mux1w (ctl_rf_alu ctlsigs) memdat r) (field prod n n)) -- where n is defined as 16 above
              pc
